...
func GzipMiddleware(logger logger.Logger) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if isGzipRequested && c.Request().Body != nil {
				...
				defer gzr.Close()
				c.Request().Body = gzr
				logger.Info("gzip request reader created", "contentEncoding", contentEncoding, "isGzipRequested", isGzipRequested)
			}
			...
			if isGzipAccepted && isAcceptedContent && c.Response().Writer != nil {
				c.Response().Header().Set(consts.HeaderContentEncoding, consts.HeaderEncodingGzip)
